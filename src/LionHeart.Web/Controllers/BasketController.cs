...
public class BasketController : Controller
{
	...
	{
		var basketWithFullData = await _basketService.GetById(basket.Id);

		if (basketWithFullData is not null)
		{
            basketWithFullData.BasketTotalPrice = basket.BasketTotalPrice;

            foreach (var product in basket.Products)
            {
                var tempProduct = basketWithFullData?.Products.FirstOrDefault(p => p.Id == product.Id);

                if (tempProduct is not null)
                {
                    tempProduct.ProductsTotalPrice = product.ProductsTotalPrice;
                    tempProduct.Quantity = product.Quantity;
                }

            }
            await _basketService.Update(basketWithFullData);
        }
